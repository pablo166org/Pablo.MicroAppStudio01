...
public abstract class FileManagementServiceIntegrationTestBase : AbpAspNetCoreIntegratedTestBase<FileManagementServiceTestsModule>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(MicroAppStudio01FileManagementServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(MicroAppStudio01FileManagementServiceModule).Assembly.FullName} assembly!");
        }
