...
public abstract class ChatServiceIntegrationTestBase : AbpAspNetCoreIntegratedTestBase<ChatServiceTestsModule>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(MicroAppStudio01ChatServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(MicroAppStudio01ChatServiceModule).Assembly.FullName} assembly!");
        }
