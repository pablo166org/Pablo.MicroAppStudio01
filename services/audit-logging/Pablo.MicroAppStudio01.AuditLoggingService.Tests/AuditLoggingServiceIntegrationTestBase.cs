...
public abstract class AuditLoggingServiceIntegrationTestBase : AbpAspNetCoreIntegratedTestBase<AuditLoggingServiceTestsModule>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(MicroAppStudio01AuditLoggingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(MicroAppStudio01AuditLoggingServiceModule).Assembly.FullName} assembly!");
        }
