...
public abstract class IdentityServiceIntegrationTestBase : AbpAspNetCoreIntegratedTestBase<IdentityServiceTestsModule>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(MicroAppStudio01IdentityServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(MicroAppStudio01IdentityServiceModule).Assembly.FullName} assembly!");
        }
